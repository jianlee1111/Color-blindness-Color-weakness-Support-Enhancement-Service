...
rgb_data.extend([[165, 42, 42], [101, 67, 33], [205, 133, 63], [210, 180, 140], [245, 245, 220]])
# GRAY family
color_names.extend(['gray', 'dark_gray', 'light_gray', 'silver', 'white', 'black'])
rgb_data.extend([[128, 128, 128], [64, 64, 64], [192, 192, 192], [192, 192, 192], [255, 255, 255], [0, 0, 0]])
# SPECIAL colors
color_names.extend(['ivory', 'cream'])
...
def create_segmented_image(img_array, labels, predictions, rgb_data, color_names):
    ...
    scale_h, scale_w = display_size[0] / h, display_size[1] / w
    for cid in cluster_names.keys():
        pixels = np.argwhere(labels == cid)
        if len(pixels) > 0:
            cy = int(np.mean(pixels[:, 0]) * scale_h)
            cx = int(np.mean(pixels[:, 1]) * scale_w)
            text = cluster_names[cid]
            bbox = draw.textbbox((0, 0), text, font=font)
            tw, th = bbox[2]-bbox[0], bbox[3]-bbox[1]
            tx, ty = cx - tw//2, cy - th//2
            draw.rectangle([tx-4, ty-4, tx+tw+4, ty+th+4], fill=(255, 255, 255, 200))
